 ...
 *   - backend must have "option server-rename" set (may be set in a defaults
 *     section to apply to all backends without per-backend configuration)
 ...
 *   - server must not have SRV_F_NON_PURGEABLE set: this covers servers
 *     tracked by another server ('track' directive), servers used by
 *     resolvers, and other configuration elements that reference the server
 *     by name. Note that sample fetches (srv_queue(), srv_is_up(), etc.)
 *     resolved to pointers at config-check time are NOT covered by this flag;
 *     an operator must audit ACL expressions manually before renaming.
 *
...
static const char *srv_update_server_name(struct server *srv, const char *new_name)
{
	...
	if (!(be->options3 & PR_O3_SRV_RENAME))
		return "Backend does not allow server renaming (add 'option server-rename' to the backend or a defaults section).\n";
	...
	/* reject if the server is referenced by other configuration elements
	 * (tracked by another server, used by a resolver, etc.): renaming would
	 * leave the running state inconsistent with the configuration text.
	 */
	if (srv->flags & SRV_F_NON_PURGEABLE)
		return "Cannot rename: server is referenced by other configuration elements.\n";
...
static int cli_parse_set_server(char **args, char *payload, struct appctx *appctx, void *private)
{
	...
	if (strcmp(args[3], "weight") == 0) {
		...
		cli_err(appctx,
			...
			"addr | agent | agent-addr | agent-port | agent-send | check-addr | check-port | fqdn | health | name | ssl | "
			"state | weight\n");
	}
 out:
	return 1;
